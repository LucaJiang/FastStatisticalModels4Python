...
Python 3.14 实验性 JIT（Copy-and-patch）理论上能给这种纯 Python 循环打折，但本地 `py314` 的 `sys._jit.is_available()` 为 `False`，所以本轮不伪造 JIT 数字。演讲里的边界应当讲清楚：JIT 是在既有解释器开销上打折，不是换算法或换运行时。
