...
IMPL_COLORS = {
    "numpy_naive": "#E65100",
    "numpy_batched": "#F57C00",
    "numpy_trick": "#0288D1",
    "numpy_trick_batched": "#1976D2",
    "multiprocessing": "#616161",
    "threads": "#455A64",
    "numba": "#2E7D32",
    "jax_perm": "#AD1457",
    "jax_trick": "#6A1B9A",
...
def plot_speedup(rows, output: Path) -> None:
    ...
    ax.set_ylabel("speedup vs NumPy-naive loop  (higher = faster)")
...
def plot_memory(rows, output: Path) -> None:
    ...
    ax.bar(x - width / 2, [impl_m[i] for i in impls], width,
           color="#0277BD", label="tracemalloc peak (parent process)")
    child_vals = [impl_ch.get(i, 0.0) for i in impls]
    if any(child_vals):
        ax.bar(x + width / 2, child_vals, width,
               color="#EF6C00", label="child processes RSS (psutil)")
    ax.set_xticks(x)
    ax.set_xticklabels([IMPL_LABEL[i] for i in impls], rotation=25, ha="right")
    ax.set_ylabel("memory (MiB, log)")
    ax.set_yscale("log")
    ax.set_title(
        f"Permutation peak memory at R={biggest_r:,} (n=10k)\n"
        "multiprocessing's children carry independent copies of the array"
    )
